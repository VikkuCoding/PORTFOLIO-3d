...
const MarqueeText = () => {
  ...
  return (
    <div
      className="absolute bottom-24 w-full overflow-hidden py-3"
      style={{
    ...
    >
      ...
      >
        {[...texts, ...texts].map((text, i) => (
          <span
            key={i}
            className="text-[#8B1A1A] text-[18px] font-medium flex items-center gap-8"
          >
            {text}
...
const Hero = () => {
  return (
    <section className="relative w-full h-screen mx-auto">
      <div
        className={`${styles.paddingX} absolute inset-0 top-[90px] max-w-7xl mx-auto flex justify-between items-center`}
      >
        {/* LEFT SIDE */}
        <div className="flex gap-5 w-1/2">
          <div className="flex flex-col justify-center items-center mt-5">
            <div className="w-5 h-5 rounded-full bg-[#8B1A1A]" />
            <div className="w-1 sm:h-80 h-40 bg-gradient-to-b from-[#8B1A1A] to-transparent" />
          </div>
        ...
        <a href="#about">
          <div className="w-[35px] h-[64px] rounded-3xl border-4 border-[#8B1A1A] flex justify-start items-start p-2">
            <motion.div
              ...
              className="w-3 h-3 rounded-full bg-[#8B1A1A]"
            />
          </div>
        </a>
      </div>
